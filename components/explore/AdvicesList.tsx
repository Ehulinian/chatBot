					...
					<View className="flex-col gap-2 mr-2">
						{adviceItems.slice(0, 3).map((item, index) => (
							<TouchableOpacity
								...
								className="bg-[#333338] rounded-xl p-3 flex-row items-center gap-2 w-[346px]  justify-start"
							>
								<LinearGradient
						...
						<View className="flex-col gap-2">
							{adviceItems.slice(3).map((item, index) => (
								<TouchableOpacity
									key={index}
									onPress={() => handleSelect(item)}
									className="bg-[#333338] rounded-xl flex-row items-center gap-2 w-[346px] p-3 justify-start"
								>
									<LinearGradient
										colors={['#448ACA', '#5C34B1']}
										style={{
